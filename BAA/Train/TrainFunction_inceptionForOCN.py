...
from BAA.models import BAAModel, Algorithm, BAAModel_Data
...
def TrainFunction_inceptionForOCN(baaModelInfo):
    """
    :param baaModelInfo: 包含训练轮数
    :return:
    """
    # # 个人版
    # base_bone_dir = os.path.join('..', '..', 'dataSet')
    # dataPath = os.path.join(base_bone_dir, 'Bone+Age+Training+Set', 'boneage-training-dataset')
    # labelPath = os.path.join(base_bone_dir, 'Bone+Age+Training+Set+Annotations', 'train.csv')
    #
    ...
    train_pic_list = baaModelInfo.get('train_pic_list')
    train_labels = baaModelInfo.get('train_labels')
    train_male_list = baaModelInfo.get('train_male_list')

    print(train_pic_list)
    print(train_labels)
    print(train_male_list)

    if len(train_male_list) != len(train_labels) | len(train_male_list) != len(train_pic_list):
        return False

    imgs_num = len(train_pic_list)
    ...
    for i in range(imgs_num):
        item = ["name", "age"]
        item[0] = train_pic_list[i]
        item[1] = str(train_labels[i])
        if train_male_list[i] == "True":
            male_data.append(item)
        if train_male_list[i] == "False":
            female_data.append(item)
        temp = item.copy()
        temp.append(train_male_list[i])
        data_list.append(temp)

    # male_data = [[name, age] for (name, age, male) in data_list if male == "True"]
    ...
    data_loader = {x: t.utils.data.DataLoader(all_dataset[x], batch_size=16, shuffle=False) for x in
                   ['train', 'val', 'test']}
    ...
    for epoch in range(EPOCH):
        ...
        for phase in ['train', 'val', 'test']:
            ...
            for step, data in enumerate(data_loader[phase], 1):
                ...
                # 转换数据类型
                # age_left = torch.tensor(age_left, dtype=torch.int64)
                # age_mid = torch.tensor(age_mid, dtype=torch.int64)
                # age_right = torch.tensor(age_right, dtype=torch.int64)
                age_left = torch.as_tensor(age_left, dtype=torch.int64)
                age_mid = torch.as_tensor(age_mid, dtype=torch.int64)
                age_right = torch.as_tensor(age_right, dtype=torch.int64)
                ...
                # Error Loss
                erroloss = (sum(abs(pred_left - pred_mid) // 2) + sum(
                    abs(pred_mid - pred_right) // 2)) * erroloss_weight
                ...
                # In paper, which index start from 1 , preid_age = pred_left + pred_age + pred_mid - 2 In this code
                # implementation , which label index start from 0, pred_age = pred_left + pred_right + pred_mid + 1
            ...
            accuracy = epoch_half_year_accuracy

            f = open('E:/PROJ_ALL/PROJ_PY/Machine-Learning/tutorial/BAA/Train/checkpoint/log.txt', 'a+')
            ...
            if phase == 'val' and month_mean < best_month:
                ...
                f = open('E:/PROJ_ALL/PROJ_PY/Machine-Learning/tutorial/BAA/Train/checkpoint/best_log.txt', 'a+')
                f.write(' Epoch: {}, month_mean: {}\n'.format(epoch, best_month))
                f.close()

            scheduler.step()

    path = 'E:/PROJ_ALL/PROJ_PY/Machine-Learning/tutorial/BAA/Models/' + baaModelInfo.get('name') + '.pth'
    torch.save(model.state_dict(), path)
    print("save model done")

    baaModel = BAAModel.objects.create(name=baaModelInfo.get('name'),
                                       epoch=baaModelInfo.get('epoch'),
                                       algorithm=Algorithm.objects.get(pk=baaModelInfo.get('algorithm')),
                                       accuracy_rate=accuracy,
                                       path=path)

    print("db model save done")

    for item in train_pic_list:
        BAAModel_Data.objects.create(model=baaModel, pic_url=item)

    print("history data save done")
